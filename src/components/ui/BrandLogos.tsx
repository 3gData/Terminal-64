// Provider brand marks. Stroke-less, scale to parent.
// Anthropic: simple "burst" star derived from their wordmark glyph.
...
export function AnthropicLogo({ size = 12, className }: LogoProps) {
  return (
    <svg
      width={size}
      height={size}
...
export function OpenAILogo({ size = 12, className }: LogoProps) {
  return (
    <svg
      width={size}
      height={size}
...
export function ProviderLogo({ provider, size = 12, className }: LogoProps & { provider: ProviderId }) {
  ...
  const label = getProviderManifest(provider).ui.shortLabel.trim().charAt(0).toUpperCase() || "?";
  return (
    <span
      className={className}
      aria-hidden="true"
      style={{
        ...
        display: "inline-grid",
        placeItems: "center",
        fontSize: Math.max(8, Math.round(size * 0.72)),
        fontWeight: 700,
        lineHeight: 1,
      }}
    >
