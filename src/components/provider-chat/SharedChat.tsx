...
function TaskIndicator({ task }: { task: { sessionId?: string; status: string } }) {
  const isThinking = useProviderSessionStore((s) => task.sessionId ? (s.sessions[task.sessionId]?.isStreaming ?? false) : false);
      ...
      <div className="sc-tasks">
        {group.tasks.map((task) => (
          <div key={task.id} className={`sc-task sc-task--${task.status}`}>
            ...
            {task.agentName && <span className="sc-task-agent">{task.agentName}</span>}
