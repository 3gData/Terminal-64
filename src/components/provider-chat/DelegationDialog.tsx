...
export default function DelegationDialog({ isOpen, onClose, onConfirm }: DelegationDialogProps) {
  const [tasks, setTasks] = useState<TaskInput[]>([
    { id: nextId++, agentName: "", description: "" },
    { id: nextId++, agentName: "", description: "" },
  ]);
  ...
  const addTask = () => {
    setTasks((prev) => [...prev, { id: nextId++, agentName: "", description: "" }]);
  };
  ...
  const updateAgentName = (id: number, agentName: string) => {
    setTasks((prev) => prev.map((t) => (t.id === id ? { ...t, agentName } : t)));
  };
  ...
  const handleConfirm = () => {
    ...
    onConfirm(
      validTasks.map((t) => ({
        ...
        ...(t.agentName.trim() ? { agentName: t.agentName.trim() } : {}),
      })),
      mergeStrategy,
      sharedContext.trim(),
    );
          ...
          <div className="del-task-list">
            {tasks.map((task, i) => (
              <div key={task.id} className="del-task-row">
                ...
                <input
                  className="del-agent-input"
                  value={task.agentName}
                  onChange={(e) => updateAgentName(task.id, e.target.value)}
