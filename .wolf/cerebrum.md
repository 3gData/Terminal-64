...
- Delegation completion parsing lives in `src/lib/delegationCompletion.ts`: normalize provider/MCP-prefixed `report_done` tool names/results there, return a `report_done` vs `idle_candidate` decision, and keep provider-specific lifecycle signals behind small sources such as `startClaudeHookCompletionSource()`.
